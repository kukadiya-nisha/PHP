...
if (isset($_SESSION['user'])) {
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        if ($quantity > 5) {
            setcookie('error', 'Cannot add more than 5 products to cart', time() + 3);
?>
            <script>
    ...
    <?php
            ...
            if (mysqli_query($con, $update_cart)) {
                ...
                setcookie('error', 'Error in updating product in cart', time() + 3);
            }
        }
    } else {
        ...
        if (mysqli_query($con, $add_to_cart)) {
            setcookie('success', 'Product added to cart', time() + 3);
        } else {
            setcookie('error', 'Error in adding product to cart', time() + 3);
        }
    }
    ?>
    <script>
